...
async function getInvoice(token: string): Promise<Invoice | null> {
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_TELEGRAM_APP_URL}/api/user/invoice?token=${token}`
    );
...
async function getUser(userId: string): Promise<User | null> {
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_TELEGRAM_APP_URL}/api/user/id/${userId}`
    );
...
async function getServer(serverId: string): Promise<Server | null> {
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_TELEGRAM_APP_URL}/api/server/${serverId}`
    );
